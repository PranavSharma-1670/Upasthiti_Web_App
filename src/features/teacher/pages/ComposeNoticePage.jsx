...
import styles from './NoticesPage.module.css'; // Correctly imports the styles as a module

function ComposeNoticePage() {
  const [formData, setFormData] = useState({ title: '', content: '', recipients: '', priority: '' });
  ...
  const loadNoticeForEditing = () => {
    ...
    if (noticeToEdit.id) {
      ...
      setFormData({
        title: noticeToEdit.title, content: noticeToEdit.content,
        recipients: noticeToEdit.recipients, priority: noticeToEdit.priority
      });
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    setIsLoading(true);
    setTimeout(() => {
      isEditMode ? updateExistingNotice() : createNewNotice();
    }, 1500);
  };

  const createNewNotice = () => {
    // Logic for creating notice remains the same
    alert('Notice sent successfully!');
    navigate('/teacher/notices'); // Corrected Path
  };

  const updateExistingNotice = () => {
    // Logic for updating notice remains the same
    alert('Notice updated successfully!');
    navigate('/teacher/notices'); // Corrected Path
  };

  const getRecipientName = (val) => ({
    'all': 'All Classes', 'class_10a_math': 'Class 10-A Mathematics',
    'class_10b_math': 'Class 10-B Mathematics'
  }[val] || '');
  
  const currentDateTime = { date: new Date().toLocaleDateString(), time: new Date().toLocaleTimeString() };

  return (
    <div className={styles['page-container']}>
      <div className={styles['breadcrumb-container']}>
        <div className={styles['breadcrumb-box']} onClick={() => navigate('/teacher/notices')}>
      ...
      <div className={styles['form-container']}>
        <form onSubmit={handleSubmit}>
          <div className={styles['form-group']}>
            <label className={styles['form-label']}>Notice Title *</label>
            <input type="text" name="title" className={styles['form-input']} value={formData.title} onChange={handleInputChange} required />
          </div>

          <div className={styles['form-group']}>
            <label className={styles['form-label']}>Notice Content *</label>
            <textarea name="content" className={styles['form-textarea']} value={formData.content} onChange={handleInputChange} required></textarea>
          </div>

          <div className={styles['form-row']}>
            <div className={styles['form-group']}>
              <label className={styles['form-label']}>Send To *</label>
              <select name="recipients" className={styles['form-select']} value={formData.recipients} onChange={handleInputChange} required>
            ...
            <div className={styles['form-group']}>
              <label className={styles['form-label']}>Priority Level *</label>
              <div className={styles['priority-options']}>
                {['high', 'medium', 'low'].map(p => (
                  <label key={p} className={`${styles['priority-option']} ${styles[`priority-${p}`]} ${formData.priority === p ? styles.selected : ''}`} onClick={() => handlePriorityChange(p)}>
                    <input type="radio" name="priority" value={p} checked={formData.priority === p} readOnly required />
          ...
          {showPreview && (
            <div className={`${styles['preview-section']} ${styles.show}`}>
              <div className={styles['preview-header']}>Preview</div>
              <div className={styles['preview-title']}>{formData.title || 'Notice Title'}</div>
              <div className={styles['preview-content']}>{formData.content || 'Notice content...'}</div>
              <div className={styles['preview-meta']}>
                <span>{currentDateTime.date}</span> <span>{currentDateTime.time}</span> <span>Teacher Name</span>
              </div>
              <div className={styles['preview-badges']}>
                {formData.priority && <span className={`${styles.badge} ${styles[`priority-${formData.priority}-badge`]}`}>{formData.priority.toUpperCase()} Priority</span>}
                {formData.recipients && <span className={`${styles.badge} ${styles['recipient-badge']}`}>{getRecipientName(formData.recipients)}</span>}
              </div>
            </div>
          )}

          <div className={styles['form-actions']}>
            <button type="button" className={`${styles.btn} ${styles['btn-secondary']}`} onClick={() => setShowPreview(!showPreview)}>
...
export default ComposeNoticePage;
...
// import React, { useState, useEffect } from 'react';
// import { useNavigate, useLocation } from 'react-router-dom';
// import './TeacherDashboardPage.module.css';
// import './NoticesPage.module.css';
