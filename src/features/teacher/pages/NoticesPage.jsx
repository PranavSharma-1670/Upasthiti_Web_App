...
function NoticesPage() {
  ...
  const [filters, setFilters] = useState({ search: '', date: '', priority: '', class: '' });
  ...
  const loadNotices = () => {
    ...
    setNotices(savedNotices.length > 0 ? savedNotices : generateSampleNotices());
  };

  const generateSampleNotices = () => {
    const sample = [
      { id: 'notice_001', title: 'Mathematics Test Schedule', content: 'The test is scheduled for next Friday.', priority: 'high', recipients: 'class_10a_math', recipientName: 'Class 10-A Mathematics', date: '2025-09-20', time: '09:30 AM', author: 'Teacher Name' },
      { id: 'notice_002', title: 'Holiday Notice', content: 'School will be closed on Monday.', priority: 'medium', recipients: 'all', recipientName: 'All Classes', date: '2025-09-19', time: '02:15 PM', author: 'Teacher Name' },
    ];
  ...
  const clearFilters = () => setFilters({ search: '', date: '', priority: '', class: '' });

  const editNotice = (noticeId) => {
    const notice = notices.find(n => n.id === noticeId);
    if (notice) {
      sessionStorage.setItem('editNotice', JSON.stringify(notice));
      navigate(`/teacher/compose-notice?edit=true`);
    }
  };
  ...
  return (
    <div className={styles['page-container']}>
      ...
      <div className={styles.header}>
        ...
        <button className={styles['compose-btn']} onClick={() => navigate('/teacher/compose-notice')}>
          Compose Notice
        </button>
      </div>
      ...
      <div className={styles['filters-section']}>
        <div className={styles['filters-controls']}>
          <input type="text" className={styles['search-box']} placeholder="Search notices..." value={filters.search} onChange={(e) => handleFilterChange('search', e.target.value)} />
          <input type="date" className={styles['date-filter']} value={filters.date} onChange={(e) => handleFilterChange('date', e.target.value)} />
          <select className={styles['priority-filter']} value={filters.priority} onChange={(e) => handleFilterChange('priority', e.target.value)}>
        ...
        {hasActiveFilters && (
          <div className={styles['filters-active']}>
            Filters applied. <button onClick={clearFilters} className={styles['clear-filters-btn']}>Clear all</button>
          </div>
        )}
      </div>

      <div className={styles['notices-list']}>
        <div className={styles['notices-header']}>
          <span>{filteredNotices.length} notice{filteredNotices.length !== 1 ? 's' : ''} found</span>
        </div>
        
        {filteredNotices.length === 0 ? (
          <div className={styles['empty-state']}>
            <div className={styles['empty-title']}>No notices found</div>
          </div>
        ) : (
            ...
            <div key={notice.id} className={styles['notice-item']}>
              <div className={styles['notice-header']}>
                <div>
                  ...
                  <div className={styles['notice-meta']}>
                    <span>{formatDate(notice.date)}</span><span>{notice.time}</span><span>{notice.author}</span>
                  </div>
                </div>
                <div className={styles['notice-actions']}>
                  <button className={`${styles['action-btn']} ${styles['edit-btn']}`} onClick={() => editNotice(notice.id)}>Edit</button>
                  <button className={`${styles['action-btn']} ${styles['delete-btn']}`} onClick={() => deleteNotice(notice.id)}>Delete</button>
                </div>
              </div>
              <div className={styles['notice-content']}>{notice.content}</div>
              <div className={styles['notice-badges']}>
                <span className={`${styles.badge} ${styles[`priority-${notice.priority}`]}`}>{notice.priority.toUpperCase()} Priority</span>
                <span className={`${styles.badge} ${notice.recipients === 'all' ? styles['all-classes-badge'] : styles['class-badge']}`}>{notice.recipientName}</span>
              </div>
            </div>
          ))
        )}
